      ...
      <div className="flex items-start justify-between gap-s4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-baseline gap-x-[10px] gap-y-[4px]">
            {/* Owner feedback (2026-08-26): dish names need to read as more
                prominent than the description — bumped from semibold/1.15rem
                to bold/1.25rem; the description below is sized down to
                match, since the owner said it's fine for that to shrink. */}
            <span className="font-display text-[1.25rem] font-bold leading-[var(--lh-title)] text-ink">
              {dish.name}
            </span>
            {dish.urdu && (
          ...
          {dish.desc && (
            <p className="mt-[4px] max-w-[48ch] font-body text-[0.8125rem] leading-[1.6] text-stone">{dish.desc}</p>
          )}
        </div>
        <DishPrice dish={dish} />
      </div>
